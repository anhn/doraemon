...
# Function to retrieve the most relevant document (Semantic Search)
def retrieve_best_document(query):
    ...
    full_text = best_doc["content"]

    # Extract 3000 tokens around the found keyword
    context_text = extract_relevant_text(full_text, query, max_tokens=3000)

    st.session_state["retrieved_context"] = context_text
    return best_doc, context_text

# Function to extract 3000 tokens around the found keyword
def extract_relevant_text(full_text, query, max_tokens=3000):
    words = full_text.split()
    query_words = query.split()

    # Find location of the first matching keyword
    match_positions = [i for i, word in enumerate(words) if any(q.lower() in word.lower() for q in query_words)]
    
    if not match_positions:
        return "No relevant keyword found in the document."

    # Find center position
    center_idx = match_positions[0]
    
    # Extract 3000 tokens around it
...
# Streamlit UI
st.title("📚 Tư vấn tuyển sinh với RAG - Semantic Search & Context Extraction")
